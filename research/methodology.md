...
The initial v0.1.0 process had a serious completeness gap: it verified every claim that authors chose to write, but nothing proved that authors had considered every item in the release notes. This allowed the repository to claim coverage through v0.10.4 while omitting `extend` and continuing to recommend behavior that v0.10.4 deprecated. The repair separates mechanical source inventory from human coverage decisions: `tooling/snapshot_release.ts` extracts every numbered item, paragraph, bullet, and fenced code block from a pinned official Markdown commit and fails on unknown sections or unparented section content; `tooling/verify_release_sources.ts` regenerates it from upstream in CI; `tooling/check_release_coverage.ts` requires every source ID to have exactly one closed decision. Schema-v2 decisions enumerate individual claims, bind each claim to unique evidence roles, and require executable evidence for every verified claim, so one POC cannot accidentally validate unrelated text grouped into the same release item. The executable workflow lives in `skills/moonbit-agent-skills-maintainer` so future release work starts with the gate rather than relying on memory.
