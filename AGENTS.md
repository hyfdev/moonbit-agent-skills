...
13. **Every new or updated eval report has a simple user-facing result.** Put it before the technical analysis and keep it even when a detailed report follows. Give every distinct task its own row named after the concrete work the agent had to complete, state the total number of distinct tasks, show exact result counts for the two conditions side by side, identify the observed model, and give the primary finding in one sentence. Name the comparison exactly (`No skills`, `Historical skill`, or the specific removed route), never just `Baseline`. Always show final task success, including ties and regressions; skill activation and reference reads are secondary evidence and must not replace it. Keep internal task IDs, claim IDs, condition names, repetitions, cells, graders, token breakdowns, and statistical analysis in the technical section. Never call cells or repetitions tasks.
