...
The repair separates three contracts. Release completeness comes from a mechanical `moonbitlang/website` inventory and schema-v2 evidence decisions. Baseline language completeness comes from a mechanical `moonbitlang/moonbit-docs` inventory that recursively follows root and nested toctrees, pinned glob expansions, and `{include}` pages. At the current pin it contains 324 documents and 1,070 document/heading items: 198 route to exact product content and 872 have explicit product boundaries (28 formal-verification items and 844 exhaustive error-code-catalog items). Each item carries a section-body fingerprint, and schema-v2 coverage records the reviewed fingerprint so a body-only upstream change reopens the decision. Discoverability is checked independently: each current actionable release decision has a reference and exact terms on one physical `Feature index` line; each routed language topic has its exact official name in the target reference and a searchable route from the owning skill. None of these routing checks proves semantics; checked documentation, fixtures, command runs, and explicitly sourced documented-only text remain the truth gate.
