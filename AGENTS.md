...
4. **Toolchain versions come from the snapshot.** Never hand-edit `moon`, `moonc`, or `moonrun` version strings. Re-pin with `vp run snapshot-toolchain --date YYYY-MM-DD`, re-run all verification, re-stamp fixtures (`vp run run-fixtures --stamp --date ...`), and update skill frontmatter to match; `tooling/check_versions.ts` cross-checks all of it.
